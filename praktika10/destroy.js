        ...
        <>
            <div className={HeadStyles.position}>
                ...
                <div className={HeadStyles.choice}>
                    ...
                    <Link to="/" className={HeadStyles.destruction}>Астероиды</Link>
                </div>
                ...
                <label className={HeadStyles.dist}>Расстояние <button className={state.units===0?HeadStyles.km:HeadStyles.moon} onClick={Km}>в километрах</button>, <button className={state.units===0?HeadStyles.moon:HeadStyles.km} onClick={Moon}>в дистанциях до луны</button></label>
            </div>
            {state.show === false ?
                (state.destroy.map((item) => <Card name={item.name} date={item.date} distance={item.distance} size={item.size} grade={item.grade} units={state.units}/>))
                :
                (state.destroy.map((item) => <>{item.grade==="опасен"?<Card name={item.name} date={item.date} distance={item.distance} size={item.size} grade={item.grade} units={state.units}/>:""}</>))
            }
