...
export function Asteroids () {
    ...
    return (
        <>
            ...
            {state.show === false ?
                (state.arr.map((item) => <Card name={item.name} date={item.date} distance={item.distance} size={item.size} grade={item.grade} units={state.units}/>))
                :
                (state.arr.map((item) => <>{item.grade==="опасен"?<Card name={item.name} date={item.date} distance={item.distance} size={item.size} grade={item.grade} units={state.units}/>:""}</>))
            }
